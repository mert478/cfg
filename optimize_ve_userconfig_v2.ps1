...
Write-Host "Ortalama Ping: $avgPing ms -> Gecikme Profili: $netTier" -ForegroundColor Magenta

# ---------------------------------------------------------
#  6b) Gercek indirme hizi (bant genisligi) testi -> rate cvar'i
# ---------------------------------------------------------
# NOT: rate cvar'i sunucunun saniyede size kac byte gonderebilecegini sinirlar.
# Bu, ping'den (gecikme) TAMAMEN FARKLI bir metriktir (bant genisligi).
# Bu yuzden gercek indirme hizini olcmeden rate belirlemek yanlis olurdu.
Write-Host "`nIndirme hizi olculuyor (kucuk bir test dosyasi indiriliyor)..." -ForegroundColor Cyan

$downloadMbps = 0
try {
    $testUrl = "https://speed.cloudflare.com/__down?bytes=4000000"  # 4 MB test dosyasi
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    $resp = Invoke-WebRequest -Uri $testUrl -UseBasicParsing -TimeoutSec 8 -ErrorAction Stop
    $sw.Stop()
    $bytesReceived = $resp.RawContentLength
    if (-not $bytesReceived -or $bytesReceived -le 0) { $bytesReceived = 4000000 }
    $seconds = $sw.Elapsed.TotalSeconds
    if ($seconds -gt 0) {
        $downloadMbps = [math]::Round((($bytesReceived * 8) / $seconds) / 1MB, 1)
    }
} catch {
    Write-Host "[UYARI] Hiz testi basarisiz (baglanti/firewall engeli olabilir), guvenli varsayilan rate kullanilacak." -ForegroundColor Yellow
    $downloadMbps = 0
}

if ($downloadMbps -ge 50) {
    $bwTier = "YUKSEK"
    $n_rate = "100000"
} elseif ($downloadMbps -ge 10) {
    $bwTier = "ORTA"
    $n_rate = "40000"
} elseif ($downloadMbps -gt 0) {
    $bwTier = "DUSUK"
    $n_rate = "25000"
} else {
    $bwTier = "OLCULEMEDI"
    $n_rate = "25000"   # olcum basarisiz olursa en guvenli/dusuk deger kullanilir
}

if ($downloadMbps -gt 0) {
    Write-Host "Olculen Indirme Hizi: ~$downloadMbps Mbps -> Bant Genisligi Profili: $bwTier" -ForegroundColor Magenta
} else {
    Write-Host "Indirme hizi olculemedi -> Bant Genisligi Profili: $bwTier (guvenli varsayilan)" -ForegroundColor Yellow
}
...
// Donanim Profili: $tier | Gecikme Profili: $netTier (ping: $avgPing ms) | Bant Genisligi: $bwTier (~$downloadMbps Mbps)
...
// --- Network (gecikme: $netTier / $avgPing ms | bant genisligi: $bwTier / ~$downloadMbps Mbps) ---
rate "$n_rate"
